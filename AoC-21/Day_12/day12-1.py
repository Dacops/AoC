def path(file):
    ...
    def path_finder(position, past_caves):
        if position=='end': return 1
        if past_caves[position]>0 and position.islower(): return 0
        if position in past_caves: past_caves[position]+=1
        path_count=0
        for path in d[position].split(','):
            path_count += path_finder(path, past_caves)
        past_caves[position]-=1
        return path_count

    return path_finder('start', past_caves)